...
> 本地 RapidOCR 识别慢且容易出错（`II` → `川`、`QDII` → `QD` 等）。
> 换成百度云端 OCR 后，**准确率接近 100%，速度提升 3-5 倍**，导入基本无需人工核对。
> **每月 1000 次免费**，个人使用完全够。
...
访问 [百度智能云控制台](https://console.bce.baidu.com) → 文字识别 → 创建应用，获取 API Key 和 Secret Key。

**2. 填入设置**

打开 `/settings` 页面：
- `OCR 后端` → 选择 **百度 OCR API**
...
系统默认使用**通用文字识别（高精度版）** `accurate_basic`，2 万字库覆盖常用字和生僻字，中英文混排识别率远高于本地引擎。
